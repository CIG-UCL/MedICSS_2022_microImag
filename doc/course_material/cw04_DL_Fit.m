...
N = 10e4;
seed1 = 123;
[modelParam, modelSignal] = generateModelTrainingSet(); % complete this
...
modelParam_odi = kappa2odi(); % complete this
...
fibredir = GetFibreOrientation(); % complete this
...
modelSignal_noisy = addNoise2Signal(); % complete this
...
numOfFeatures = ; % complete this
numOfOutputs = ; % 2 are theta and phi 
...
trainParam = ; % complete this
[xTrain, yTrain, xValidation, yValidation] =   trainValSplit(); % complete this
...
% the training GUI.
rng(456);
net = trainNetwork(); % complete this
...
subplot(1,3,1), plot_gt_vs_prediction(, 'NDI (ficvf)'); % complete this
subplot(1,3,2), plot_gt_vs_prediction(, 'ODI'); % complete this
subplot(1,3,3), plot_gt_vs_prediction(, 'FWF (fiso)'); % complete this

% How do the plots look?
% ANSWER: 

figure,plot_gt_vs_prediction(, 'NDI VS FWF'); % complete this
figure,plot_gt_vs_prediction(, 'ODI VS FWF'); % complete this
figure,plot_gt_vs_prediction(, 'ODI VS NDI'); % complete this

%}

%% 4.5 Extend to an unseen voxel
...
noddiNet_gcc_odi([1 3 4]) = ; % complete this
noddiNet_gcc = odi2kappa(); % complete this
...
loglik_gccnet = getFitRicianLogLik(); % complete this
fig_noddiQC_gccnet = QualityOfFit(); % complete this
...
%% 4.6 Extend the analysis to in-vivo data
...
roi_norm = ; % complete this

% Let's use the network to predict the parameters
tic
fittedParams_dl = predict(); % complete this
dl_fitTime = toc;
...
save(fullfile('coursework_outputs', 'cw4.mat'))
