...
load(voxel_dwi_data_path, 'csfVox'); % complete this

% Describe the "csfVox" viriable and its dimensions. To what they correspond?
% ANSWER: "csfVox" is an arrey of integers. The dimension is 1x81. 81 corresponds
% to the number of different acquisitions used in this particular
% acquisition.
...
% variable
csfVox = double(csfVox); % complete this
...
bval = importdata(bval_path); % complete this
bvec_path = fullfile('..', '..', 'NODDI_example_dataset/NODDI_protocol.bvec');
bvec = importdata(bvec_path); % complete this

% What is the content of the variables? Can you guess what the values refer
% to?
% ANSWER: the bvalues loaded in "bval" are 81. There are 3 different
% values: 0, 700, 2000 (s/mm2) corresponding to non, moderate and high diffusion
% weightings. The b-vectors in bvec are 81 triplets. Each triplet
% corresponds to a different direction of diffusion encoding in the space
% described in cartesian coordinates.

% OPTIONAL: if you have extra time try to plot the b-vectors as dots on the
% unitary sphere.

bvec_fig = figure('position',[100 100 500 500]);
plot3(bvec(1,:),bvec(2,:),bvec(3,:),'*r');
axis([-1 1 -1 1 -1 1]);
axis vis3d;
rotate3d
...
plot(1:nAcq, csfVox, '-o', 'LineWidth', 2) % complete this
ylabel('DW signal')
xlabel('Acquisitions')
...
% ANSWER: This example has chosen a voxel in the ventricle (CSF). 
...
figure(fig_csf);
hold on
plot(1:nAcq, bval, '--o', 'LineWidth', 1); % complete this
...
load(voxel_dwi_data_path, 'gccVox');  % complete this
gccVox = double(gccVox);  % complete this

% Crteate exactly the same plot as before but for this voxel
fig_gcc = figure('Position', [400 300 1200 600], 'color', [ 1 1 1]);
plot(1:nAcq, gccVox, '-o', 'LineWidth', 2)  % complete this
ylabel('DW signal')
xlabel('Acquisitions')
...
% ANSWER: this is a voxel in the genu of the corpus callosum, a 
...
% the b-vectors
dotProduct =  abs(fibredir*bvec);  % complete this

% complete the code to produce the plot of Signal vs dot product
fig_gcc_dot = figure('Position', [400 100 600 600], 'color', [ 1 1 1]);
plot(dotProduct, gccVox, 'x', 'LineWidth', 2);  % complete this
ylabel('DW signal')
xlabel('$\vert \hat{g} \cdot \hat{n} \vert$', 'Interpreter', 'latex')
title('GCC voxel time series')

% What do you see? What does it means?
% ANSWER: The strong correlation in the data highlights the expected
% signal dependence on the gradient direction, a hallmark of diffusion 
% anisotropy.

% Try to do the same with the csf voxel. What do you expect?
% ANSWER: no correlation as there is no anisotropy.

fig_csf_dot = figure('Position', [400 100 600 600], 'color', [ 1 1 1]);
plot(dotProduct, csfVox, 'x', 'LineWidth', 2);  % complete this
ylabel('DW signal')
xlabel('$\vert \hat{g} \cdot \hat{n} \vert$', 'Interpreter', 'latex')
title('CSF voxel time series')
...
% ANSWER: The black dashed lines represent the 13 different b=0. They are
% represented as lines as they don't have any angular dependence. Each red
% cross corresponds to an acquisition with b=700 s/mm2 and a different
% gradient direction. The blue crosses corresponds to b=2000 s/mm2.
% You can see that the blue crosses tends to have lower signal value than
% the red ones. However, for highly anisotropic voxels there is a high
% dependence on the acquisition direction.
